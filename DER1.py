#!/usr/bin/env python3
"""
DER-style dynamic expert routing demo (Ollama experts + PPO router)

Adds:
- tqdm progress bars for PPO training (per environment step) and evaluation
- keeps HF/Transformers warning spam suppressed, but you still see clear progress
...
# --- Silence noisy HF / tokenizers logs BEFORE importing transformers/bert_score ---
...
# Extra HF logging suppression (safe if transformers is installed via bert_score deps)
try:
...
# SB3 callback for tqdm progress
try:
...
# -----------------------------
# Metric (BERTScore F1) + memoization
# -----------------------------
_BERT_F1_CACHE: Dict[Tuple[str, str], float] = {}


def bert_f1(pred: str, ref: str) -> float:
    pred_n = normalize_yes_no((pred or "").strip().lower())
    ref_n = normalize_yes_no((ref or "").strip().lower())
    key = (pred_n, ref_n)
    if key in _BERT_F1_CACHE:
        return _BERT_F1_CACHE[key]

    P, R, F1 = bertscore([pred_n], [ref_n], lang="en", verbose=False)
    val = float(F1[0].detach().cpu().item())
    _BERT_F1_CACHE[key] = val
    return val
...
# -----------------------------
# tqdm callback for SB3 training
# -----------------------------
class TqdmCallback(BaseCallback):
    def __init__(self, total_timesteps: int, desc: str = "Training PPO", disable: bool = False):
        super().__init__()
    ...
    def _on_step(self) -> bool:
        if self.pbar is not None:
            # Each callback step corresponds to env steps collected
            self.pbar.update(1)
        return True
...
def evaluate(
    ...
    def iter_with_pbar(it, desc):
        if tqdm is None:
            return it
        return tqdm(it, desc=desc, total=len(eval_items))
    ...
    def all_oracle():
        ps, calls, costs, lats = [], [], [], []
        for (q, ref) in iter_with_pbar(eval_items, "Eval: all_experts_oracle"):
            best_p = -1.0
            total_cost = 0.0
            total_lat = 0.0
            for exp in experts:
...
def main():
    ...
    if tqdm is None:
        print("WARNING: tqdm is not installed. Progress bars disabled.")
    ...
    model = PPO(
        policy="MlpPolicy",
        env=vec_env,
        verbose=0,  # tqdm provides the main feedback; set to 1 if you also want SB3 logs
        seed=args.seed,
        n_steps=128,
        batch_size=64,
        learning_rate=3e-4,
    ...
    cb = TqdmCallback(total_timesteps=args.ppo_steps, desc="PPO training (env steps)", disable=(tqdm is None))
    t0 = time.time()
    ...
    q, ref = eval_items[0]
    ans_prev = ""
    trace_iter = range(args.tmax)
    if tqdm is not None:
        trace_iter = tqdm(trace_iter, desc="Trace steps", total=args.tmax)

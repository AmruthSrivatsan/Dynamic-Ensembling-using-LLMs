"""
DERFinal.py — Demo implementation of DER-style dynamic ensembling (paper-inspired)
=================================================================================

Paper target:
  "Efficient Dynamic Ensembling for Multiple LLM Experts" (DER)

What this script implements (concept-to-code map)
-------------------------------------------------
(1) Multiple LLM "experts" ............................ OllamaExpert / ExpertSpec
(2) Sequential refinement with Knowledge Transfer ..... FIRST_PROMPT_TEMPLATE / KTP_TEMPLATE
(3) Router learns which expert to call next ........... PPO router trained on DEREnv (MDP)
(4) Reward trades off quality vs cost ................. DEREnv.step(): reward = quality + beta*delta - alpha*cost
(5) Early stopping (stop when "good enough") .......... p0 threshold + gamma stop bonus
(6) Fair cost handling ................................ per-expert latency normalization (median latency)

Important practical notes (demo realism vs paper specifics)
----------------------------------------------------------
- We use PPO for the router. PPO is a convenient, explainable choice for a demo; the paper may
  use different training details.
- We use BoolQ and evaluate *accuracy* (yes/no exact match). This makes "quality" sparse (0/1),
  so delta-quality shaping (beta) helps learning.
- Cost is wall-clock latency. We normalize costs per expert so the router does not collapse
  into "always pick the fastest model".

Run examples
------------
Fast iteration:
  python DERFinal.py --train_samples 120 --eval_samples 120 --ppo_steps 2000 --show_der_qa 5

Stronger run:
  python DERFinal.py --train_samples 400 --eval_samples 200 --ppo_steps 8000 \
    --alpha 0.05 --beta 1.0 --gamma 0.8 --tmax 3 --show_der_qa 10 --save_router der_router.zip

Interactive MoE mode starts at the end (type /quit to exit).
"""
...
class ExpertSpec:
    model: str
    # After latency calibration, we set:
...
class OllamaExpert:
    def __init__(self, spec: ExpertSpec, host: str):
        self.spec = spec
    ...
    def answer(self, question: str, prev_answer: str) -> Tuple[str, float, int, str]:
        ...
        # Token proxy (Ollama may include eval_count; fallback to small proxy)
...
def measure_median_latencies(
    experts: List[OllamaExpert], items: List[Tuple[str, str]], n_calib: int, seed: int
) -> Dict[str, float]:
    rnd = random.Random(seed)
    calib = items[:]
    rnd.shuffle(calib)
...
def apply_latency_normalization(experts: List[OllamaExpert], med: Dict[str, float]) -> None:
    # IMPORTANT: cost_weight = 1 / median_latency
    # so normalized_cost = latency * cost_weight = latency / median_latency.
    for exp in experts:
...
class DEREnv(gym.Env):
    ...
    def step(self, action: int):
        ...
        # No-repeat makes refinement more meaningful (prevents stuck "spam same expert" loops).
        ...
        y, latency, tokens, _raw = exp.answer(self._q, self._ans_prev)
...
# =============================================================================
# 6) Clean tqdm training callback (prevents garbled duplicated lines)
# =============================================================================
class TqdmCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.pbar is None:
            return True
        # SB3 uses self.num_timesteps; update by delta to keep bar accurate and clean.
...
def der_pick_action(der_policy: PPO, embedder: SentenceTransformer, q: str, ans_prev: str) -> int:
    ...
    for i, _exp in enumerate(experts):
        results.append(
            run_multistep_route(
                ...
                p0=p0,
                cost_mode=cost_mode,
                no_repeat=False,  # allow repeats (this baseline is measuring that)
    ...
    if der_policy is not None:
        results.append(
            run_multistep_route(
                ...
                cost_mode=cost_mode,
                no_repeat=no_repeat,
                route_fn=lambda q, a, t, last: der_pick_action(der_policy, embedder, q, a),
            )
        )

        # Per-question traces for slides
        for idx, (q, ref) in enumerate(eval_items[: max(0, int(show_der_qa))]):
            ...
            for t in range(tmax):
                ...
                tot_c += c
                tot_l += lat
                trace.append({"t": t, "pick": exp.spec.model, "ans": y, "gold": ref, "acc": int(acc),
                              "latency": float(lat), "tokens": int(tok), "cost": float(c)})
                ans_prev = y
                last_action = a
                if acc >= p0:
                    break
            der_samples.append({
                "idx": idx, "q": q, "gold": ref, "final": ans_prev, "correct": int(quality_accuracy(ans_prev, ref)),
                "calls": len(trace), "total_latency": float(tot_l), "total_cost": float(tot_c), "trace": trace,
            })
...
def print_der_samples(samples: List[Dict[str, Any]], max_chars: int = 260) -> None:
    ...
    for s in samples:
        ...
        for st in s["trace"]:
            print(f"      t={st['t']} pick={st['pick']} ans={st['ans']} acc={st['acc']} "
...
def main():
    ...
    ap.add_argument("--experts", type=str, nargs="*", default=[
        # Keep 3 light-ish experts; you may swap based on what runs best on your RTX 3060
    ...
    if args.load_router:
        ...
        router = PPO(
            ...
            learning_rate=3e-4,
            gamma=0.95,
            ent_coef=0.01,      # encourage exploration early; helpful for routing diversity
